...
//
//
//public class BuildCircuitsPartitioned extends BuildCircuits{
//	public BuildCircuitsPartitioned(Args options, GateLibrary gate_library, Roadblock roadblock) {
//		 super(options, gate_library, roadblock);
//	}
//
//    @Override
//    public void buildCircuits(){
//    	//PartitionCircuit in, assigned gates on sub_lc out
//    	
//        logger = Logger.getLogger(getThreadDependentLoggername());
//        System.out.println("Enumerating logic circuits w/ partitioned circuits code...");
//
...
//        PartitionCircuit pc = new PartitionCircuit(get_unassigned_lc());
//        LogicCircuit lc = pc.getParent_lc();
//        List<LogicCircuit> sublcs = pc.getSub_lcs().get(1); //work w/ single set of LogicCircuits
//        int total_num_lc_gates = 0;
//        
//        //keys are integers of total number of gates, values are [internal gate number, subgraph number]
//        HashMap<Integer, List<Integer>> gate_subcircuit_map = new HashMap<Integer, List<Integer>>();
//        //int subgraph_num = 0;
//        for(int subgraph_num=0; subgraph_num < sublcs.size(); ++subgraph_num) {
//        		for(int gate_num=0; gate_num <= sublcs.get(subgraph_num).get_logic_gates().size(); ++gate_num) {
//        			gate_subcircuit_map.put(total_num_lc_gates, Arrays.asList(gate_num, subgraph_num));
//        			total_num_lc_gates += 1;
//        		}
//        }
//        
...
//            	
//            		/*
//            		 * 1. Create a hashmap w/ integer keys ranging from 0, (total number of all gates in subgraph) representing 
//            		 * indices of various logic gates and integer values denoting which subcircuit a gate is contained w/in
//            		 * 2. Grab any random index --> check if terminal gate in given subgraph (check hashmap)
//            		 * 		2a) if so, substitute w/ any gate in terminal gates set or swap w/ another terminal subgraph gate
//            		 *		2b) if not, continue to step 3. 
//            		 * 3. Grab any other random index --> check if gate is in same subcircuit
//            		 * 		3a) if so, swap gates
//            		 * 		3b) if not, substitute w/ gate not in subcircuit
//            		 * 	
//            		 * 
//            		 */
//            	
...
//                
//                int A_index = generator.nextInt(gate_subcircuit_map.keySet().size());
//                LogicCircuit A_sublc = sublcs.get(gate_subcircuit_map.get(A_index).get(1));
//                
//                //if(gate_subcircuit_map.get(A_index).get(0) == A_sublc.get_output_gates().get(0)) {
//                //		break;
//                //}
//                
